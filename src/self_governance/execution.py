...
def dispatch_swarm_execution(
    ...
    for agent in agents:
        ...
        if "role_0" in role or "dev" in role:
            traces.append({"agent_role": role, "status": dev_res["status"], "output": dev_res["output"]})
        elif "role_1" in role or "qa" in role:
            traces.append({"agent_role": role, "status": test_res["status"], "output": test_res["output"]})
        else:
            traces.append({"agent_role": role, "status": "completed", "output": f"Execution output for {role}"})
