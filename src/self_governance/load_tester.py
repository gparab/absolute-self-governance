...
async def send_request(client: httpx.AsyncClient, url: str, headers: dict) -> float:
    ...
    try:
        response = await client.post(
            ...
            json={
                "action": "opened",
                "issue": {"title": "Scale testing", "body": "Benchmarking pipeline"},
            },
            headers=headers,
            timeout=30.0,
        )
...
async def run_load_test(
    url: str, headers: dict, concurrent_requests: int = 10, total_requests: int = 50
):
    print(
        f"Starting pipeline load test: sending {total_requests} total requests with concurrency of {concurrent_requests}..."
    )
    ...
    async with httpx.AsyncClient() as client:
        ...
        print(
            f"Success Rate: {len(valid_latencies) / total_requests * 100:.1f}% ({len(valid_latencies)} successes, {failures} failures)"
        )
