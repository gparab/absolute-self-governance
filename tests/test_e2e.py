    ...
    if hasattr(config, "dict"):
        ...
        try:
            ...
            if hasattr(config, "swarm"):
                data = {
                    "swarm": [
                        {"role": a.role, "prompt": a.prompt} for a in config.swarm
                    ]
                }
            else:
                data = config
...
def test_consensus_temp_scaling():
    ...
    initial_roster = ["agent_A", "agent_B"]
    res = run_consensus(
        initial_roster, B=3, target_tau=9.5, initial_temp=1.0, gamma=0.1
    )
...
def test_workload_unstable_consensus():
    ...
    res = run_consensus(
        initial_roster, B=5, target_tau=9.5, initial_temp=1.0, gamma=0.1, delta=1.0
    )
...
def test_workload_recovery_on_failed_iteration(tmp_path):
    ...
    nudger_resumed.trigger_succession(
        "status: COMPLETED\ncandidates:\n  - agent_recovered\n"
    )
